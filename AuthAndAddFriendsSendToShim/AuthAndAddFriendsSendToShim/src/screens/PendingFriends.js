...
class PendingFriends extends Component {
  ...
  fetchFriends(e) {
    ...
    const requestOptions = {
      ...
      body: JSON.stringify({username: this.props.nameOfUser}),
    };
  ...
  addFriend(e) {
    const requestOptions = {
      method: 'POST',
      headers: {Accept: 'application/json', 'Content-Type': 'application/json'},
      body: JSON.stringify({username: this.props.nameOfUser, pending: e}),
    };
  ...
  render() {
    ...
    const Item = ({title}) => (
      <View>
        <Text
          style={styles.body}
          onPress={this.getListViewItem.bind(this, title)}>
          {title}
        </Text>
      </View>
    );

    const renderedItem = ({item}) => <Item title={item.title} />;

    const myListEmpty = () => {
      return (
        <View style={{alignItems: 'center'}}>
          <Text style={styles.body}>You have no friends</Text>
        </View>
      );
    };
    ...
    return (
      <SafeAreaView>
        <FlatList data={this.state.data} renderItem={renderedItem} />
      </SafeAreaView>
    );
  }
}
